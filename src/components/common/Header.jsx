...
const NavLinks = [
  { name: `HOME`, link: `/` },
  { name: `THE EDITION`, link: `/the-trail` },
...
const Header = () => {
  ...
  return (
    <>
        ...
        <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 w-fit px-5 h-auto gap-5.5 justify-center items-center">
          {
            NavLinks.map((nav, i) => {
              return (
                <TransitionLink key={i} href={nav.link} >
      ...
      <div
        className={`fixed top-0 left-0 w-full h-[100dvh] bg-[#0987D8] z-50 flex md:hidden flex-col justify-between px-6 pt-24 pb-10 transition-opacity duration-500 ease-in-out ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
          }`}
      >
        {/* Nav Links */}
        <div className="flex flex-col gap-5">
          {
            NavLinks.map((nav, i) => (
              <div key={i} className="overflow-hidden -my-2">
                <TransitionLink href={nav.link} onClick={() => setIsOpen(false)}>
                  <span
                    className={`block text-white uppercase text-[2.2rem]! leading-none py-2 cursor-pointer transition-all duration-500 ease-out ${isOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
                      }`}
        ...
        {/* CTA */}
        <div
          className={`w-full flex gap-1.5 cursor-pointer transition-all duration-500 ease-out ${isOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
            }`}
