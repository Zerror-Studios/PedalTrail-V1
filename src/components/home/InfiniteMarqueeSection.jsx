    ...
    <section className="bg-white text-black min-h-screen py-12 sm:py-16 overflow-hidden flex flex-col justify-between gap-[10vh] sm:gap-[15vh] md:gap-[20vh] pt-[8vh] sm:pt-[10vh]">
      <div className="px-5 md:px-10 flex flex-col gap-5">
        ...
        <p className="Inter font-light! capitalize max-w-2xl">
          ...
          designed to seamlessly blend intense competition with world-<br className="hidden md:block" />
          class hospitality and cultural immersion.
        </p>
      </div>
      ...
      <div className="w-full overflow-hidden relative">
        <div ref={marqueeRef} className="flex gap-2 w-max">
          {[...items, ...items].map((item, index) => (
            <div key={index} className="flex flex-col w-[75%] sm:w-1/2  shrink-0 group">
              {/* Top Text label */}
              <div className="text-xs md:text-sm Inter tracking-wider  mb-3 uppercase">
                {item.topText}
              </div>
